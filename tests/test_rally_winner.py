"""Rally winner: which court side won, from annotation to aggregated segment.

Covers the producer-side path: the editor's save persists ``winner`` (and only
...
per-frame ``winner_probs`` into one winning side per rally — averaged over the
segment's final ``WINNER_TAIL_S`` seconds only, since earlier frames cannot
know the outcome.
"""
...
class LoadRalliesCarriesSideTest(unittest.TestCase):
    def test_side_reaches_every_consumer(self):
        ...
        self.assertEqual(rallies[0]["winner"], "near")
        self.assertIsNone(rallies[1]["winner"])
...
class SegmentAggregationTest(unittest.TestCase):
    def _tick(self, t: float, probs=None) -> dict:
        ...
        if probs is not None:
            event["winner_probs"] = probs
        return event

    def test_tail_decides_the_winner(self):
        # Early frames vote right, the WINNER_TAIL_S tail votes left: the tail wins.
        events = [
            self._tick(
                t / 2,
                [0.8, 0.1, 0.05, 0.05]
                if t / 2 >= 10.0 - WINNER_TAIL_S
        ...
        self.assertEqual(seg["winner"], "left")
        self.assertEqual(seg["winner_score"], 0.8)

    def test_winnerless_events_yield_winnerless_segment(self):
        events = [self._tick(t / 2) for t in range(0, 21)]
        (seg,) = events_to_rally_segments(
            events, native_fps=30, min_score=0.5, max_gap_s=2.0, min_duration_s=4.0
        )
        self.assertNotIn("winner", seg)
        self.assertNotIn("winner_score", seg)
...
class TrainingLabelsCarrySideTest(unittest.TestCase):
    def test_write_training_labels_attaches_side(self):
        ...
        self.assertEqual(events[0]["winner"], "far")
