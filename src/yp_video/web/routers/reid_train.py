...
- ``/export`` + ``/start`` turn those labels into a CLIP-ReIdent dataset
  (see reid/dataset.py), which is what the training stage will consume.

Model training itself is deliberately not here yet: with two labeled videos
there is nothing to train on that would generalize, and no way to tell whether
it helped. Hence the baseline first.
"""
...
from yp_video.reid import dataset, evaluate, sessions, store
from yp_video.reid.clip_reident import clip_reident_available
from yp_video.reid.embedder import build_embedders, threshold_calibration
from yp_video.web.job_helpers import fail_job_from_exc
from yp_video.web.jobs import JobStatus, job_manager, make_progress_callback
...
def _active_job() -> dict | None:
    for job in job_manager.jobs.values():
        if job.type == JOB_TYPE and job.status in (JobStatus.PENDING, JobStatus.RUNNING):
            return job.to_dict()
    return None
...
def status() -> dict:
    ...
    return {
        ...
        "clip_reident_available": clip_reident_available(),
...
class ReidExportRequest(BaseModel):
    ...
    seed: int = 42
    #: Symlink into crops/ instead of copying — see dataset.write_export.
    link: bool = True
    #: Link the background-suppressed crops the masked embedders saw.
    masked: bool = False
...
async def start(req: ReidExportRequest) -> dict:
    ...
    job = job_manager.create_job(
        ...
        name=f"ReID dataset ({plan.counts['n_rows']} crops)",
    )

    async def run_job() -> None:
        loop = asyncio.get_running_loop()
        on_progress = make_progress_callback(job.id, loop, "Linking crops ({done}/{total})")
        try:
            await job_manager.update_job(job.id, status=JobStatus.RUNNING, message="Writing dataset...")
            # Filesystem work, not GPU work — taking gpu_lock here would block
            # training for the sake of creating symlinks.
            result = await asyncio.to_thread(
                dataset.write_export, plan, root, link=req.link, on_progress=on_progress
            )
            await job_manager.update_job(
                job.id,
                status=JobStatus.COMPLETED,
                progress=1.0,
                message=f"{result['n_images']} crops ({result['link_mode']}) -> {name}",
