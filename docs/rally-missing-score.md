...
判定：rally span `[start, end]` 內最後的動作事件不是 `score`。共 215 筆。
...
| 0518小窩季打 1 | 4 | 1:02 | 1:12 | receive | serve → receive → set → spike → receive |
| 0518小窩季打 1 | 6 | 1:41 | 1:48 | receive | serve → receive |
| 0518小窩季打 1 | 9 | 2:57 | 3:07 | spike | serve → receive → set → spike |
| 0518小窩季打 1 | 11 | 3:35 | 3:44 | serve | serve |
| 0518小窩季打 1 | 21 | 7:38 | 8:01 | receive | set → spike → receive → set → spike → receive |
| 0518小窩季打 1 | 26 | 9:50 | 10:07 | spike | receive → set → receive → receive → set → spike |
| 0518小窩季打 1 | 28 | 10:30 | 10:39 | spike | serve → receive → set → spike |
| 0518小窩季打 1 | 31 | 11:31 | 11:45 | receive | set → spike → receive → set → spike → receive |
| 0518小窩季打 1 | 39 | 14:24 | 14:31 | serve | serve |
| 0518小窩季打 1 | 45 | 16:09 | 16:15 | receive | serve → receive |
| 0518小窩季打 2 | 3 | 0:37 | 0:48 | spike | serve → receive → set → spike |
| 0518小窩季打 2 | 13 | 4:06 | 4:19 | receive | receive → receive → receive → set → spike → receive |
| 0518小窩季打 2 | 16 | 4:52 | 5:00 | serve | serve |
| 0518小窩季打 2 | 20 | 6:09 | 6:30 | receive | set → spike → receive → set → spike → receive |
| 0518小窩季打 2 | 21 | 6:45 | 6:58 | spike | serve → receive → receive → receive → set → spike |
| 0518小窩季打 2 | 25 | 8:07 | 8:20 | receive | serve → receive → set → spike → receive |
| 0518小窩季打 2 | 29 | 9:43 | 9:56 | spike | serve → receive → receive → receive → set → spike |
| 0518小窩季打 2 | 33 | 11:31 | 11:39 | receive | serve → receive |
| 0518小窩季打 2 | 35 | 12:04 | 12:29 | spike | receive → set → spike → receive → set → spike |
| 0518小窩季打 2 | 36 | 12:43 | 13:02 | receive | set → receive → receive → set → spike → receive |
| 0518小窩季打 2 | 37 | 13:12 | 14:11 | set | receive → receive → set → spike → receive → set |
| 0518小窩季打 2 | 43 | 15:49 | 16:24 | receive | receive → spike → receive → set → receive → receive |
| 0518小窩季打 2 | 49 | 18:36 | 18:46 | receive | serve → receive → set → spike → receive |
| 0518小窩季打 3 | 5 | 1:16 | 1:34 | receive | set → receive → receive → set → spike → receive |
| 0518小窩季打 3 | 9 | 2:33 | 2:48 | receive | set → spike → receive → set → spike → receive |
| 0518小窩季打 3 | 14 | 4:20 | 4:29 | receive | serve → receive → set → spike → receive |
| 0518小窩季打 3 | 18 | 5:43 | 5:59 | receive | receive → receive → receive → set → spike → receive |
| 0518小窩季打 3 | 24 | 7:46 | 8:04 | receive | set → spike → receive → set → spike → receive |
| 0518小窩季打 3 | 27 | 9:26 | 9:36 | receive | serve → receive |
| 0518小窩季打 3 | 31 | 11:00 | 11:28 | spike | receive → set → spike → receive → set → spike |
...
| 0518小窩季打 3 | 25 | 8:09 | 8:33 | receive | 16.9s | set → spike → receive → set → spike → receive |
...
| 2025-10-04_G3_臺中連莊_vs_桃園雲豹飛將_set1 | 13 | 5:53 | 5:59 | receive | 2.5s | serve → receive → set → score → receive |
...
| 0518小窩季打 1 | 3 | 0:50 | 0:58 | 2 | 1.27s |
| 0518小窩季打 1 | 37 | 13:39 | 13:46 | 2 | 0.30s |
| 0518小窩季打 1 | 38 | 13:55 | 14:18 | 2 | 1.60s |
| 0518小窩季打 2 | 1 | 0:01 | 0:20 | 3 | 0.53s |
| 0518小窩季打 2 | 4 | 0:54 | 1:05 | 2 | 0.13s |
| 0518小窩季打 2 | 10 | 2:53 | 3:03 | 2 | 0.17s |
| 0518小窩季打 2 | 27 | 8:44 | 9:24 | 2 | 1.50s |
| 0518小窩季打 2 | 28 | 9:29 | 9:36 | 2 | 2.20s |
| 0518小窩季打 3 | 4 | 0:49 | 1:12 | 2 | 1.57s |
| 0518小窩季打 3 | 12 | 3:39 | 3:53 | 2 | 0.83s |
| 0518小窩季打 3 | 33 | 11:51 | 12:18 | 2 | 0.53s |
...
| Bulgaria 🇧🇬 vs. Japan 🇯🇵 ｜ VNL 2025 - Full Match ｜ Week 1_set1 | 1 | -1:59 | 0:05 | 2 | 0.24s |
